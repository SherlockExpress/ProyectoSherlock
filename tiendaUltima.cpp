...
using namespace std;
// Jorq -> agregaa setters
// Richon hace cambios con la privacidad
class Producto {
private:
    int id;
    string nombre;
...
// Santy hace cambios con estructura del programa 
...
void mostrarMenu() {
    ...
    cout << "6. Buscar productos por precio maximo\n";
    cout << "7. Salir\n";
...
// Leo -> implemento una funcion q busque por precio maximo
// se podria hacer con busqueda binaria, pero no tiene mucho sentido en este caso
// ya q tendriamos que ordenar la lista en O(Nlog(n))
// y ademas las busqueda binaria en O(log(n))
// y O(N) < O(NLog(n)+Log(n))
void buscarProductosPorPrecioMaximo(unordered_map<int, Producto>& catalogo) {
    double precioMax;
    ...
    for (auto& par : catalogo) {
        if (par.second.getPrecio() <= precioMax) {
            filtrados.push_back(par.second);
        }
    }

    if (filtrados.empty()) {
        cout << "No hay productos con precio menor o igual a $" 
             << fixed << setprecision(2) << precioMax << ".\n";
    } else {
        cout << "Productos con precio <= $" 
             << fixed << setprecision(2) << precioMax << ":\n";
...
int main() {
    ...
    catalogo[8] = Producto(8, "Cocaina", 55.40);
    catalogo[9] = Producto(9, "Marihuana", 83.94);
    catalogo[10]= Producto(10,"Chaer",100.22);
    catalogo[11]= Producto(11,"Pomelo", 3.99);
    catalogo[12]= Producto(12,"Tarea",6.90);
    catalogo[13]= Producto(13,"Laser",4.99);
    catalogo[14]= Producto(14,"Pancho",1.99);
    catalogo[15]= Producto(15,"Milanesa",2.99);
    catalogo[16]= Producto(16,"Pizza de 4 quesos",5.99);
    ...
    do {
        ...
        switch (opcion) {
            ...
            case 3: {
                if (pedidoActual.empty()) {
                    ...
                    for (int i = 0; i < (int)pedidoActual.size(); ++i) {
                        ...
                        total += pedidoActual[i].getPrecio();
                    }
            ...
            case 6: {
                buscarProductosPorPrecioMaximo(catalogo);
                break;
            }
            case 7: {
